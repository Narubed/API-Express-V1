...
exports.delete = async (req, res) => {
  const id = req.params.id;
  try {
    CutAround.findByIdAndRemove(id, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    CutAround.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.findByPartner = async (req, res) => {
  try {
    const id = req.params.id;
    CutAround.find({ cut_parners_id: id })
      .then(async (data) => {
        res.send({ data, message: "success", status: true });
      })
      .catch((err) => {
